...
class BrnPopupListWindow {
  ...
  static void showPopListWindow(context, GlobalKey popKey,
    ...
    double maxWidth = 550;
    double maxHeight = 200;
    Color borderColor = const Color.fromARGB(255, 176, 175, 175);
    ...
    return data.map((f) {
      return GestureDetector(
          ...
          child: Container(
              ...
              decoration: BoxDecoration(
                color: Colors.transparent,
                border: data.indexOf(f) == data.length - 1
                    ? Border.all(color: Colors.transparent)
                    : const Border(
                        bottom: BorderSide(color: Color(0x33CCCCCC))),
              ),
              padding: EdgeInsets.only(
                  top: data.indexOf(f) == 0 ? 0 : 10,
                  bottom: data.indexOf(f) == data.length - 1 ? 0 : 10),
