...
import 'package:journal/components/bruno/src/constants/brn_constants.dart';
...
class BrnSelectionEntity {
  ...
  static BrnSelectionEntity fromMap(Map<String, dynamic> map) {
    ...
    if (map['children'] != null && map['children'] is List) {
      entity.children = [...(map['children'] as List).map((o) => BrnSelectionEntity.fromMap(o))];
    }
  ...
  BrnSelectionEntity.fromJson(Map<dynamic, dynamic>? map)
    ...
    extMap = map['ext'] ?? {};
    children = [...(map['children'] ?? []).map((o) => BrnSelectionEntity.fromJson(o))];
  ...
  void configDefaultValue() {
    if (children.isNotEmpty) {
      ...
      if (children.where((_) => _.isSelected).toList().isEmpty) {
        List<BrnSelectionEntity> rangeItems = children.where((_) {
  ...
  List<BrnSelectionEntity> selectedLastColumnList() {
    ...
    if (children.isNotEmpty) {
      ...
      for (BrnSelectionEntity firstEntity in children) {
  ...
  List<BrnSelectionEntity> selectedList() {
    if (BrnSelectionFilterType.more == filterType) {
  ...
  /// 校验 range 类型的数据是否合法
  bool isValidRange() {
    if (filterType == BrnSelectionFilterType.range ||
        filterType == BrnSelectionFilterType.dateRange ||
        filterType == BrnSelectionFilterType.dateRangeCalendar) {
